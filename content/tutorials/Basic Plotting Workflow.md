...
- the **Products** tab, where you can find all data accessible to SciQLop.
...
Choosing between the two options simply consists in where you drop the product once selected from the product tree. In the following, we have chosen to plot the magnetic field vector components in the GSM coordinate system, underneath the magnetic amplitude.
...
>Deleting a panel or a plot from the currently displayed view is done by pressing the `Delete` key on the selected item.
